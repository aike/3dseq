...
var ThreePiece = function(id, w, h, useDirtyFlag) {
	...
	for (var i = 0; i <= 30; i++) {
		var j = i - 15;
		grid.data.push({obj:'line',x:j*2,y:-6,z:-30,tx:j*2,ty:-6,tz:30,col:0x44EE44});
		grid.data.push({obj:'line',x:-30,y:-6,z:j*2,tx:30,ty:-6,tz:j*2,col:0x44EE44});
	}
...
ThreePiece.prototype.removeHook = function(id) {
	this.hooks.splice(id, 1) ;
}

ThreePiece.prototype.clearHook = function() {
	this.hooks = [];
}
...
ThreePiece.prototype.rotate = function(speed) {
	...
	var self = this;
	this.rotateId = this.addHook(function() {
		self.rotAngle += speed * 0.005;
		self.objs['world'].rotation.y = self.rotAngle;
	});
}

ThreePiece.prototype.stopRotate = function() {
	this.removeHook(this.rotateId);
}
